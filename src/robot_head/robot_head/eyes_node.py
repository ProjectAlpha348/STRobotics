...
class EyesNode(Node):
    ...
    def _on_cmd(self, msg: String):
        ...
        # --------------------------
        # pupil=center
        m = re.match(r"^pupil\s*=\s*center\s*$", cmd, re.IGNORECASE)
        if m:
            self._set_look(0, 0)
            return

        # pupil=x:10,y:-5  (pixel)
        m = re.match(r"^pupil\s*=\s*x\s*:\s*(-?\d+)\s*,\s*y\s*:\s*(-?\d+)\s*$", cmd, re.IGNORECASE)
        if m:
            x = int(m.group(1))
            y = int(m.group(2))
            self._set_pupil_px(x, y)
            return

        # pupil=0.6,-0.2  (normalizzato)
        m = re.match(r"^pupil\s*=\s*([-+]?\d*\.?\d+)\s*,\s*([-+]?\d*\.?\d+)\s*$", cmd, re.IGNORECASE)
        if m:
            nx = float(m.group(1))
            ny = float(m.group(2))
            self._set_pupil_norm(nx, ny)
            return
    ...
    def _set_pupil_px(self, x: int, y: int):
        # identico comportamento di _set_look (pixel)
        self._set_look(x, y)

    def _set_pupil_norm(self, nx: float, ny: float):
        """
        nx, ny in [-1..+1]
        mappa su pixel usando gli stessi limiti di _set_look:
          x: [-20..+20], y: [-12..+12]
        """
